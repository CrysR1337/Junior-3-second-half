...
class Cluster(object, ):
    ...
    def draw_network(self):
        ...
        nx.draw(self.state_G, pos, with_labels=True, cmap=plt.get_cmap('Accent'), node_color=self.state_cluster_id,
                node_size=200)
        plt.show()
        ...
        for i in range(0, node_number + server_number):
            ...
            for j in range(0, node_number + server_number):
                if i != j and (self.state_xcor[i] - self.state_xcor[j]) ** 2 + (
                        self.state_ycor[i] - self.state_ycor[j]) ** 2 <= transmit_range_square:
                    node_link.append(1)
                else:
                    node_link.append(0)
    ...
    def set_cluster_id(self):
        ...
        for i in range(server_number, node_number + server_number):
            ...
            closest_distance_square = float('inf')
            for j in range(0, server_number):
                communication_distance_square = ((self.state_cluster_core_xcor[j] - self.state_xcor[i]) ** 2 + (
                            self.state_cluster_core_ycor[j] - self.state_ycor[i]) ** 2) * self.weight_cluster[j] ** 2
    ...
    def transmit_flow_in_network(self):
        ...
        for i in range(server_number, total_node_number):
            ...
            event_distance_square = (self.state_xcor[i] - self.state_event_xcor) ** 2 + (
                        self.state_ycor[i] - self.state_event_ycor) ** 2
        ...
        for k in range(0, len(state_cluster_size)):
            total_state_balance = total_state_balance + (
                        1 - abs(state_cluster_size[k] - mean_state_balance_data) / mean_state_balance_data)
        ...
        for k in range(0, len(state_cluster_path_length)):
            total_state_balance = total_state_balance + (
                        1 - abs(state_cluster_path_length[k] - mean_state_balance_com) / mean_state_balance_com)
    ...
    def action_route(self, action, tick):
        ...
        if action_id == 1:
            if self.state_cluster_core_ycor[cluster_id] + move_step_length_header <= deploy_range_y:
                self.state_cluster_core_ycor[cluster_id] = self.state_cluster_core_ycor[
            ...
            if self.state_cluster_core_xcor[cluster_id] + move_step_length_header <= deploy_range_x:
                self.state_cluster_core_xcor[cluster_id] = self.state_cluster_core_xcor[
                                                               cluster_id] + move_step_length_header
        elif action_id == 4:
    ...
    def set_network_to_optimal_in_exp(self, cluster_head_state):
        self.state_cluster_core_xcor = cluster_head_state[:server_number]
        self.state_cluster_core_ycor = cluster_head_state[server_number:2*server_number]
        self.weight_cluster = cluster_head_state[2 * server_number:3 * server_number]
...
class ExperienceReplay(object):
    ...
    def _build_model(self):
        ...
        if (Dueling):
            model.add(Dense(model_output_size + 1, activation='softmax'))
            model.add(Lambda(lambda i: K.expand_dims(i[:, 0], -1) + i[:, 1:] - K.mean(i[:, 1:], keepdims=True),
                             output_shape=(model_output_size,)))
        else:
        ...
        for tick in range(0, game_time):  # In every epoch, we move the event for game_time.
            ...
            if always_best_experience_method:
                ...
                if np.isnan(best_method_state[int(round(env.state_event_xcor/grid_size_x) * (grid_num_x+1) + round(env.state_event_ycor/grid_size_y)) ,0]):
                    ...
                    x, y = int(round(env.state_event_xcor/grid_size_x)), int(round(env.state_event_ycor/grid_size_y))
            ...
            elif DQN_and_RNN_method:
                if using_combined_probability:
                    if mem_state == []:
                        if np.isnan(
                        ...
                        behavior_prediction = RNN_model.predict(
                            np.reshape(env.trace_input[total_tick], (-1, time_steps, 2)), batch_size=1)                            #Here implementing cross-probability
                    ...
                    if mem_state == []:
                        ...
                        behavior_prediction = RNN_model.predict(np.reshape(env.trace_input[total_tick], (-1, time_steps, 2)), batch_size=1)
                        print(np.argmax(behavior_prediction))
                        network_state = env.network_state
                        network_state = np.asarray(list(network_state) + list(behavior_prediction.reshape(total_behavior_dim,)))
                ...
                if np.isnan(best_method_in_mem):
                    ...
                    dim_transfer = int(round(env.state_event_xcor / grid_size_x) * (grid_num_x+1) + round(
                        env.state_event_ycor / grid_size_y))
            ...
            print("==>> Epoch {:03d}/{} | Tick {}/{} | Reward {}/{}".format(a, epoch, tick, game_time, round(reward, 2),
                                                                            round(sta_max_reward, 2)))

            if flag_gameover_mode == 1:
                if game_over == 1:
                    break
            if total_tick >= len(env.trace_input)-1:
                total_tick = 0
